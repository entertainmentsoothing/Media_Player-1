...
@interface AudioPlayerController ()<AIAudioMixerManagerDelegate, AIAudioNodeDelegate, VerticalSliderBarDelegate, VerticalEQBarDelegate, ReverbOptionsViewDelegate, GADInterstitialDelegate>
...
- (void)playAudio:(DeviceMediaModel*)audio inList:(NSArray<DeviceMediaModel*>*)playlist{
    ...
    if(![[NSUserDefaults standardUserDefaults] boolForKey:@"OEPNED_RATING_AUDIOPLAYER"]) {
        [[NSUserDefaults standardUserDefaults] setBool:YES forKey:@"OEPNED_RATING_AUDIOPLAYER"];
...
- (void)minimizeIfNeeded{
    if ([AppDefine sharedInstance].isInAD == YES) {
        [AppDefine sharedInstance].isInAD = NO;
        return;
    }
